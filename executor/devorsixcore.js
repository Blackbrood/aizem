...
break
 ...
 //================================================//
   case "addsrv": {
if (!isDeveloper) return xxreply(`You are not my owner`)
let s = text.split(',');
if (s.length < 7) return xreply(`> *Incorrect format!*
...
if (res.errors) return xxreply(JSON.stringify(res.errors[0], null, 2))
...
let t = text.split(',');
if (t.length < 2) return xreply(`use:\n${prefix + command} user,number`)
let username = t[0];
...
let ctf = `❗Hello @${m.sender.split('@')[0]} , ᴸᴼᴿᴰ SEYORI™ Just Gave You Access To The Following Account Panel > >
...
if (res.errors) return xreply(JSON.stringify(res.errors[0], null, 2))
let server = res.attributes
...
case '2gb': {
if(!isDeveloper){
xreply(`You are not my owner`)
}
if (!isDeveloper) return xreply(mess.owner)
let t = text.split(',');
if (t.length < 2) return xreply(`Format salah!\nPenggunaan:\n${prefix + command} user,nomer`)
let username = t[0];
let u = m.quoted ? m.quoted.sender : t[1] ? t[1].replace(/[^0-9]/g, '') + '@s.whatsapp.net' : m.mentionedJid[0];
let name = username
let egg = global.eggsnya
